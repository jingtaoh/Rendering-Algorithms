...
void test_xformed_camera_image()
{
    ...
    Camera camera({
        ...
        {"fdist", 1.f},
        {"transform", {{"from", {5.0, 15.0, -25.0}}, {"to", {0.0, 0.0, 0.0}}, {"up", {0.0, 1.0, 0.0}}}},
    });
...
void test_ray_sphere_intersection()
{
    ...
    auto   material = make_shared<Lambertian>(json{{"albedo", 1.f}});
    ...
    auto   transform = Transform::axisOffset(Vec3f(2.0, 0.0, 0.0), // x-axis
...
void test_sphere_image()
{
    ...
    Camera camera({
        {"vfov", 90.f},
        {"resolution", Vec2i(ray_image.sizeX(), ray_image.sizeY())},
        {"fdist", 1.f},
        {"transform", {{"from", {5.0, 15.0, -25.0}}, {"to", {0.0, 0.0, 0.0}}, {"up", {0.0, 1.0, 0.0}}}},
    });
    ...
    for (auto y : range(ray_image.height()))
    {
        for (auto x : range(ray_image.width()))
        {
            // TODO: Look in camera.h and implement camera.generate_ray
...
void test_materials()
{
    ...
    if (lambert_material->scatter(ray, hit, lambert_attenuation, lambert_scattered))
    {
        ...
        Vec3f   correct_direction(1.206627e+00f, 3.683379e-01f, -8.104229e-01f);
    ...
    if (metal_material->scatter(ray, hit, metalAttenuation, metalScattered))
    {
        Vec3f   correct_origin      = surface_point;
        Color3f correct_attenuation = surface_color;
        Vec3f   correct_direction(2.697650e-01, 9.322242e-01, -2.421507e-01);
...
void test_recursive_raytracing()
{
    ...
    Camera camera({
        ...
        {"resolution", Vec2i(ray_image.sizeX(), ray_image.sizeY())},
        {"fdist", 1.f},
        {"transform", {{"from", {1.9, 0.8, -3.5}}, {"to", {1.9, 0.8, 0.0}}, {"up", {0.0, 1.0, 0.0}}}},
    });
    ...
    spdlog::info("Average number of intersection tests per ray: {} ", float(intersection_tests) / float(rays_traced));
